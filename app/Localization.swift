...
extension Loc {
    ...
    static let zh: [String: String] = [
        ...
        "When enabled, each Side-button external voice attempt is saved from physical press to release as raw audio plus capture metadata, including very short presses and no-text attempts. Clipboard is preserved only as unattributed Raw evidence; only a re-verified Accessibility target can mark text as observed. Missing text is not treated as failure. Native Voice does not need to be enabled.": "开启后，每次侧键外部语音尝试都会从物理按下到松开保存原始音频和采集元数据，包括极短按键和没有文字的情况。剪贴板只作为未归属的 Raw 证据保存；只有重新验证为同一编辑目标的辅助功能文字才能标记为已观察。没有文字不视为失败，也无需开启原生语音。",
        "Credential Broker validation failed. This signed build is refusing to fall back to plaintext credential storage. Rebuild or re-sign the App and Broker together.": "凭据 Broker 验证失败。这个已签名版本会拒绝降级到明文凭据存储；请让 App 与 Broker 使用一致身份重新构建或签名。",
        "Credential Broker validation failed. This signed build will not fall back to plaintext credential storage.": "凭据 Broker 验证失败。这个已签名版本不会降级到明文凭据存储。",
        "Output mode": "输出模式",
