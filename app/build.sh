...
if [ $? -eq 0 ]; then
    ...
    echo ""
    echo "Next: stage and sign a development candidate outside /Applications:"
    echo "  HYPERVIBE_SIGN_MODE=developer ./create_app_bundle.sh"
    echo ""
    echo "Do not run the raw ./HyperVibe binary or the staged candidate while the installed"
    echo "stable HyperVibe is running; real-device testing is a separate rollback-protected step."
else
    echo ""
    echo "✗ Build failed!"
    exit 1
